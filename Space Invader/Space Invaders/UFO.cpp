...
Ufo::Ufo(std::mt19937_64& i_random_engine) :
	...
	animation(UFO_ANIMATION_SPEED, 2 * BASE_SIZE, "graphics/Ufo.png"),
	explosion(EXPLOSION_ANIMATION_SPEED, 2 * BASE_SIZE, "graphics/ExplosionBig.png")
{
	reset(1, i_random_engine);

	for (unsigned char a = 0; a < POWERUP_TYPES; a++)
	{
		powerup_animations.push_back(Animation(POWERUP_ANIMATION_SPEED, BASE_SIZE, "graphics/Powerup" + std::to_string(static_cast<unsigned short>(a)) + ".png"));
	}
}
