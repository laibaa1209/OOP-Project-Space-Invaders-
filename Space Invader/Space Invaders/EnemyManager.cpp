	...
	shoot_distribution(0, ENEMY_SHOOT_CHANCE)
{
	...
	enemy_bullet_texture.loadFromFile("graphics/EnemyBullet.png");

	enemy_bullet_sprite.setTexture(enemy_bullet_texture);

	for (unsigned char a = 0; a < ENEMY_TYPES; a++)
	{
		enemy_animations.push_back(Animation(1 + move_pause, BASE_SIZE, "graphics/Enemy" + std::to_string(static_cast<unsigned short>(a)) + ".png"));
	}
}
...
void EnemyManager::draw(sf::RenderWindow& i_window)
{
	...
	for (Enemy& enemy : enemies)
	{
		...
		//SoundBuffer buffer;
		//buffer.loadFromFile("sound/invaderexplode.ogg");
		//Sound invader;
		//invader.setBuffer(buffer);
		//invader.play();
