...
int main(){
	...
	Texture bgd;
	bgd.loadFromFile("graphics/background.png");
	Sprite background;
	...
	window.setView(sf::View(sf::FloatRect(0, 0, SCREEN_WIDTH * screenRatio, SCREEN_HEIGHT * screenRatio)));

	sf::Sprite background_sprite;
	sf::Sprite powerup_bar_sprite;

	sf::Texture background_texture;
	background_texture.loadFromFile("graphics/Background2.png");

	sf::Texture font_texture;
	font_texture.loadFromFile("fonts/Font.png");

	sf::Texture powerup_bar_texture;
	powerup_bar_texture.loadFromFile("graphics/PowerupBar.png");
	...
	while (1 == window.isOpen())
	{
		...
		Button playButton(sf::Vector2f(200, 50), "Play", font, 30, sf::Color::Green, sf::Color::White);
		playButton.setPosition(sf::Vector2f(300, 250));

		Button quitButton(sf::Vector2f(200, 50), "Quit", font, 30, sf::Color::Red, sf::Color::White);
		quitButton.setPosition(sf::Vector2f(300, 350));
		...
		while (FRAME_DURATION <= lag)
		{
			...
			if (FRAME_DURATION > lag )
			{
				...
				draw_text(0.04f * BASE_SIZE, 0.04f * BASE_SIZE, "Level: " + std::to_string(level), window, font_texture);
				draw_text(0.05f * BASE_SIZE, 0.05f * BASE_SIZE, "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tLives: ", window, font_texture);
